...
| 3  | Identity abuse                 | Recording              | `principal_identity` · `rsa_cert_key_id` · Ed25519 signing                                  | Every artifact proves which agent acted via Ed25519-signed identity. Cannot be forged without signing-key compromise. Production artifacts use FIPS 140-2 Level 3 HSM signing; design partner phase uses software signing with the same algorithm. |
...
| **Observability** (Datadog, Arize, LangSmith)            | Mutable application logs. Not legal-grade evidence.                  | AgentOS: Ed25519 signed (software in design-partner phase, HSM at FIPS 140-2 Level 3 in production), S3 Object Lock COMPLIANCE (WORM), RFC 3161 timestamped. Survives adversarial examination. |
...
| -------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| OCC Bulletin 2026-13 | Rescinded prior MRM framework; agentic AI deferred to forthcoming interagency RFI | `model_version_hash` · `policy_version_hash` · `signing_attestation` · `principal_identity` |
...
The companion SP 800-53 Rev 5 mapping is at [`docs/sp800-53-mapping.md`](https://github.com/getagentos/sp800-53-mapping/blob/main/docs/sp800-53-mapping.md).

Contact: `swapan@getagentos.ai` · [getagentos.ai](https://getagentos.ai)

---

*Published May 23, 2026. Updated June 1, 2026. Apache License, Version 2.0.*
